...
import { Search, Filter, Plus, Edit, Trash2, Eye, Calendar, User, Loader, Download, MapPin } from 'lucide-react';
import GalleryModal from '../components/GalleryModal';
import { AuthContext } from '../context/AuthContext';

const BranchAdminGallery = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/gallery/stats?branchId=${user?.branch?._id}`);
      const data = await response.json();
      if (response.ok) {
        setStats(data);
      }
  ...
  const exportData = () => {
    const csvContent = [
      ['Title', 'Description', 'Category', 'Created By', 'Created Date'],
    ...
    a.href = url;
    a.download = `${user?.branch?.name}_gallery_export_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    ...
    return (
      <div className='p-6'>
        <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center'>
          <p className='text-yellow-800'>Branch information not available. Please contact administrator.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className='p-6'>
      <div className='mb-6'>
        ...
        <p className='text-gray-600'>Manage gallery images for <span className='font-semibold'>{user.branch.name}</span></p>
      </div>
      ...
      <div className='bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6'>
        <div className='flex items-center gap-3'>
          ...
          <div>
            <p className='font-semibold text-blue-800'>{user.branch.name}</p>
            <p className='text-sm text-blue-600'>{user.branch.address?.street}, {user.branch.address?.city}</p>
          </div>
        </div>
      </div>
      ...
      {Object.keys(stats.categoryStats).length > 0 && (
        <div className='bg-white p-4 rounded-lg shadow-sm border mb-6'>
          <h3 className='font-semibold text-gray-800 mb-3'>Images by Category</h3>
          ...
          <div className='text-center py-20'>
            <div className='text-gray-500 mb-4'>
              {searchTerm || (selectedCategory !== 'All') 
                ...
                : 'No images in your branch gallery yet.'}
            </div>
            <button
              onClick={handleAddImage}
...
export default BranchAdminGallery;
